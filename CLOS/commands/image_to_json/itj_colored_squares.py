    ...
    def json_to_text(scling = 1, shrink = 1, json2 = '{"name": "lol", "w": 0, "h": 0, "pix":[[],[]]}', bw = False, rc = 0, gc = 1, bc = 2, ac = 3, brgb = color.brgb, rgb = color.rgb, r = color.r):
        ...
        while i+shrink <= scaling[1]:
            ...
            while i2+shrink <= scaling[0]:
                ...
                if bw:
                    rgb1 = int((val[0]+val[1]+val[2])/3)
                    if val[rc] == val[gc] and val[gc] == val[bc] and val[rc] <= 128:
                        pval = pval+'⬛'
                    elif val[rc] == val[gc] and val[gc] == val[bc] and val[rc] <= 128:
                        pval = pval+'⬜'
                    elif val[rc] > (val[gc]+val[bc])/2:
                        pval = pval+'🟥'
                    elif val[gc] > (val[rc]+val[bc])/2:
                        pval = pval+'🟩'
                    elif val[bc] > (val[gc]+val[rc])/2:
                        pval = pval+'🟦'
                else:
                    if val[rc] == val[gc] and val[gc] == val[bc] and val[rc] <= 128:
                        pval = pval+'⬛'
...
ij2 = itj.img_to_json(1,1,str('/home/justus/Bilder/PixelArt/Creeper.png'))
