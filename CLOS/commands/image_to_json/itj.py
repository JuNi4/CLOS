...
class itj():
    ...
    def img_to_text(scling = 1, shrink = 1, img = 'img.png', bw = False, rc = 0, gc = 1, bc = 2, ac = 3, rgb = color.rgb, r = color.r):
        scling = int(scling)
        shrink = int(shrink)
        img = Image.open(img)
        img = img.convert('RGB')
        ...
        while i+shrink <= scaling[1]:
            ...
            while i2+shrink <= scaling[0]:
                ...
                if bw:
                    rgb1 = (val[rc]+val[gc]+val[bc])/3
                    pval = pval+rgb(rgb1,rgb1,rgb1)+'██'*scling
                else:
                    pval = pval+rgb(val[rc]+val[gc]+val[bc])+'██'*scling
                i2 += shrink
            i += shrink
            print(pval+r)
        img.close()
    ...
    def json_to_text(scling = 1, shrink = 1, json2 = '{"name": "lol", "w": 0, "h": 0, "pix":[[],[]]}', bw = False, rc = 0, gc = 1, bc = 2, ac = 3, rgb = color.rgb, r = color.r):
